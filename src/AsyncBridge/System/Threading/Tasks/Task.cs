    ...
    public class Task : IAsyncResult, IDisposable
    {
        ...
        internal ExecutionContext CapturedContext
        {
            get
            {
                ...
                {
                    return m_contingentProperties?.m_capturedContext ?? ExecutionContextEx.Default;
                }
            }
            set
            {
                ...
                else if (value != ExecutionContextEx.Default) // not the default context, then inflate the contingent properties and set it
                {
        ...
        private void HandleException(Exception unhandledException)
        {
            ...
            if (exceptionAsOce != null && IsCancellationRequested &&
                m_contingentProperties.m_cancellationToken == exceptionAsOce.GetCancellationToken())
            {
        ...
        internal void RecordInternalCancellationRequest(CancellationToken tokenToRecord, object cancellationException)
        {
            ...
            if (cancellationException != null)
            {
                ...
                Debug.Assert(oce.GetCancellationToken() == tokenToRecord,
        ...
        private void RunOrQueueCompletionAction(ITaskCompletionAction completionAction, bool allowInlining)
        {
            ...
            {
                ThreadPool.UnsafeQueueUserWorkItem(state => ((CompletionActionInvoker)state).ExecuteWorkItem(), new CompletionActionInvoker(completionAction, this));
            }
        }
        ...
        internal static Task<TResult> FromCancellation<TResult>(OperationCanceledException exception)
        {
            ...
            var task = new Task<TResult>();
            bool succeeded = task.TrySetCanceled(exception.GetCancellationToken(), exception);
            Debug.Assert(succeeded, "This should always succeed on a new task.");
            return task;
        }
        ...
        public static Task Delay(int millisecondsDelay, CancellationToken cancellationToken)
        {
            ...
            {
                promise.Timer = new Timer(state => ((DelayPromise)state).Complete(), promise, (uint)millisecondsDelay, unchecked((uint)Timeout.Infinite));
            }
        ...
        private bool TrySetFromTask(Task task, bool lookForOce)
        {
            ...
            switch (task.Status)
            {
                ...
                case TaskStatus.Faulted:
                    ...
                    {
                        result = TrySetCanceled(oce.GetCancellationToken(), oceEdi);
                    }
                    else
                    {
